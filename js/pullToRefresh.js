var refresher = {
	...
	scrollIt: function(parameter, pullDownEle, pullDownOffset, pullUpEle, pullUpOffset) {
		eval(
			parameter.id + "= new iScroll(parameter.id,{\
					 useTransition: true,\
					 vScrollbar: false,\
					 topOffset: pullDownOffset,\
					 onRefresh: function () {refresher.onRelease(pullDownEle,pullUpEle);},\
					onScrollMove: function () {refresher.onScrolling(this,pullDownEle,pullUpEle,pullUpOffset);},\
					onScrollEnd: function () {refresher.onScrollEnd(pullDownEle,parameter.pullDownAction,pullUpEle,parameter.pullUpAction);}\
				 })"
			);
	},
